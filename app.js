  ...
  el("v-courses").innerHTML = `
    ...
    <article class="card pad">
      ...
      <details class="howto" open style="margin-top:10px">
        ...
        <div class="howtobody">
          <div class="allocwrap">
          ${COURSE_ALLOC.terms.map(t => `<table class="alloctable">
            <tr><th class="at" colspan="2">${esc(t.name)}</th></tr>
            ${[["必修科目", t.req], ["選修科目", t.ele]].map(([lab, list]) => list.length
              ? `<tr><th class="ah">${lab}</th><th class="ah cr">學分</th></tr>
                 ${list.map(([n, c]) =>
                   `<tr><td>${esc(n)}</td><td class="cr">${c}</td></tr>`).join("")}`
              : "").join("")}
          </table>`).join("")}
          </div>
          <div class="hint" style="margin-top:8px">
